...
public class OpenFromExternalEventAction implements IWorkbenchWindowActionDelegate
{
	...
	/**
	 * Pattern to match a java file name followed by line number e.g. <code>OpenFromClipboardAction.java : 21</code>
	 */
	...
	/**
	 * Pattern to match a member (field or method) of a type e.g. <code>OpenFromClipboardAction#run</code>,
	 * <code>Worker#run</code>
	 */
	...
	/**
	 * Pattern to match a method e.g. <code>OpenFromClipboardAction#run(IAction)</code>, <code>Worker#run()</code>
	 */
	...
	{
		final String inputText = m_inputText;
	 ...
	 *           if fetching the Java element's source range fails
	 */
	...
	private static void doTypeSearch(String typeName, final List<Object> matches, IProgressMonitor monitor)
	{
		...
		try
		{
			searchEngine.searchAllTypeNames(packageName == null ? null : packageName.toCharArray(),
																			packageName == null ? SearchPattern.R_EXACT_MATCH : getSearchFlags(),
	 ...
	 *          the remaining Work
	 */
	private static void doMemberSearch(String memberName,
																		 final List<Object> matches,
																		 boolean searchForMethods,
																		 boolean searchForConstructors,
																		 boolean searchForFields,
																		 ...
																		 int work)
	{
		...
		try
		{
			...
			if (searchForMethods)
			{
				doMemberSearch(searchEngine,
											 memberName,
											 IJavaSearchConstants.METHOD,
											 scope,
											 requestor,
											 progress.newChild(workPerSearch));
			}
	...
	private static void doMemberSearch(SearchEngine searchEngine,
																		 String memberName,
																		 int searchFor,
																		 IJavaSearchScope scope,
																		 SearchRequestor requestor,
																		 SubMonitor progressMonitor) throws CoreException
	{
